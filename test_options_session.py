...
def make_contract(symbol='AAPL260319C00207000', strike='207.00', exp='2026-03-19'):
    c = MagicMock()
    c.symbol = symbol
...
def build_agent(
    ...
    mock_odc = MagicMock()
    contract_sym = (contracts[0].symbol if contracts else 'AAPL260319C00207000')
    ...
    with (
        patch('scripts.options_agent.TradingClient', return_value=mock_tc),
        patch('scripts.options_agent.OptionHistoricalDataClient', return_value=mock_odc),
        patch('scripts.options_agent.yf.Ticker', return_value=mock_ticker),
        patch('scripts.options_agent.get_trading_decision', return_value=ollama_resp),
        patch('scripts.options_agent.parse_decision', return_value=lora_dec),
    ):
        agent = OptionsAgent()
...
class TestHappyPath(unittest.TestCase):
    ...
    def _patch_and_run(self):
        ...
        with (
            patch('scripts.options_agent.yf.Ticker', return_value=MagicMock(
                history=MagicMock(return_value=make_price_df())
            )),
            patch('scripts.options_agent.get_trading_decision',
                  return_value='BUY_CALL. Confidence: 0.85.'),
            patch('scripts.options_agent.parse_decision',
...
class TestPositionManagement(unittest.TestCase):
    ...
    def test_take_profit_closes_position(self):
        ...
        pos = make_option_position('AAPL260319C00207000', qty=2, unrealized_plpc=0.55)
    ...
    def test_stop_loss_closes_position(self):
        ...
        pos = make_option_position('TSLA260319P00250000', qty=1, unrealized_plpc=-0.55)
        tc = self._run_manage([pos])
        tc.submit_order.assert_called_once()
        order_arg = tc.submit_order.call_args[0][0]
        self.assertEqual(order_arg.side.value, 'sell')
    ...
    def test_normal_position_not_closed(self):
        ...
        pos = make_option_position('MSFT260319C00380000', qty=1, unrealized_plpc=-0.20)
        tc = self._run_manage([pos])
    ...
    def test_both_exits_in_same_session(self):
        ...
        pos_tp = make_option_position('AAPL260319C00207000', qty=2, unrealized_plpc=0.55)
        pos_sl = make_option_position('TSLA260319P00250000', qty=1, unrealized_plpc=-0.55)
        pos_ok = make_option_position('MSFT260319C00380000', qty=1, unrealized_plpc=0.10)
...
class TestInsufficientCapital(unittest.TestCase):
    ...
    def test_no_trade_when_options_fully_allocated(self):
        ...
        agent, tc, _ = build_agent(
            equity='1000.00',
            positions=[
                make_option_position('AAPL260319C00207000', qty=1, unrealized_plpc=0.05),
            ],
        )
...
class TestFindOptimalOption(unittest.TestCase):
    ...
    def test_returns_best_contract(self):
        ...
        contracts = [
            make_contract('AAPL260319C00200000', '200.00', '2026-03-19'),
...
class TestOptionDecisionLogic(unittest.TestCase):
    ...
    def _decide(self, response_text, parse_result, momentum=0.05):
        ...
        with (
            patch('scripts.options_agent.get_trading_decision', return_value=response_text),
            patch('scripts.options_agent.parse_decision', return_value=parse_result),
        ):
            return self.agent.get_ai_options_decision(analysis)
