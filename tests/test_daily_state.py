...
class TestDailyState:
    def test_save_and_load_returns_value(self, db_path):
        import db as _db
        _db.save_daily_start_equity('stock', 10_000.0, db_path)
        result = _db.load_daily_start_equity('stock', db_path)
        assert result == 10_000.0

    def test_load_returns_none_for_missing_bot(self, db_path):
        import db as _db
        _db.save_daily_start_equity('stock', 10_000.0, db_path)
        result = _db.load_daily_start_equity('options', db_path)
    ...
    def test_upsert_overwrites_same_day(self, db_path):
        import db as _db
        _db.save_daily_start_equity('stock', 10_000.0, db_path)
        _db.save_daily_start_equity('stock', 12_500.0, db_path)
        result = _db.load_daily_start_equity('stock', db_path)
    ...
    def test_stock_and_options_are_independent(self, db_path):
        import db as _db
        _db.save_daily_start_equity('stock',   10_000.0, db_path)
        _db.save_daily_start_equity('options',  5_000.0, db_path)
        assert _db.load_daily_start_equity('stock',   db_path) == 10_000.0
        assert _db.load_daily_start_equity('options', db_path) ==  5_000.0
